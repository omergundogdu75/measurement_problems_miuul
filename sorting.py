...
from sklearn.preprocessing import MinMaxScaler  # standartlaştırma için kullanacağız
...
####################
# 1-5 aralığında ölçeklendiriyor.
df["purchase_count_scaled"] = MinMaxScaler(feature_range=(1, 5)). \
    fit(df[["purchase_count"]]). \
    transform(df[["purchase_count"]])
df.describe().T

df["comnment_count_scaled"] = MinMaxScaler(feature_range=(1, 5)). \
...
#Verilen ratingler üzerincden ağırlık bir şekilde olasılıksal ortalama hesaplar
...
#5 ve 1. indexi seçip bar scora göre sıraladık
...
# r = vote average # ort puan
# v = vote count # ilgili filmin mevcut oy sayısı
# M = minimum votes required to be listed in the Top 250 # gereken oy sayısı
# C = the mean vote across the whole report (currently 7.0) # tüm filmlerin genel ort.
